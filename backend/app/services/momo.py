...
momo_provider: MomoProvider = MockMomoProvider()
...
async def create_or_get_momo_transaction_async(
    ...
    *,
    ...
    transaction = MomoTransaction(
        ...
        status="PENDING",
        request_payload=json.dumps({"phone": phone, "amount": amount, "loan_id": loan_id}, ensure_ascii=False),
    )
    db.add(transaction)
    await db.flush()
    return transaction
...
def complete_momo_transaction(
    ...
    *,
    ...
    transaction.response_payload = json.dumps({"reference": reference, "message": message}, ensure_ascii=False)
