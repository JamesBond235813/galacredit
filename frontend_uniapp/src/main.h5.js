...
function installH5Uni() {
  ...
  // 安卓 WebView/UniApp 运行时可能已经预先注入 window.uni；即便如此，
  // 自定义 navigator 标签仍需要注册 H5 路由点击代理，否则卡片看似可点但不会跳转。
  if (window.uni) {
    if (!window.__gcNavigatorClickInstalled) {
      window.__gcNavigatorClickInstalled = true
  ...
  window.uni = {
    ...
    getStorageSync: readStoredValue,
    ...
    request: async (options) => {
      ...
      try {
        ...
        const headers = { ...(options.header || {}), ...(token ? { Authorization: `Bearer ${readStoredValue('token')}` } : {}) }
    ...
    // H5/WebView 中不能使用 alert：它会阻塞页面并在滑块验证后遮住验证码输入框。
    // 统一派发轻量提示事件，由 App.h5.vue 渲染为非阻塞 toast。
    showToast: ({ title, duration = 2600 } = {}) => {
      if (title) window.dispatchEvent(new CustomEvent('gc-toast', { detail: { title, duration } }))
    },
