...
2. 登录 HBuilderX 的 DCloud 账号后执行 `bash scripts/pack-app-plus.sh`；也可通过“发行 → App-Android/iOS-云打包”操作。脚本使用安心打包（本机签名），避免传统云端签名失败。当前采用同签名 JKS 兼容证书用于手机覆盖升级，不是新创建的上架证书。私钥在被忽略的 `android_native/.local/` 中，勿提交。
